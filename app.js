...
// ========== SEARCHABLE STATIC FEATURES ==========
const searchableFeatures = [
    { label: 'Dashboard', keywords: ['dashboard', 'home', 'overview', 'stats'], tab: 'dashboard', icon: '📊' },
    { label: 'Add Trade', keywords: ['add trade', 'new trade', 'buy', 'sell', 'entry', 'order'], tab: 'addTrade', icon: '📈' },
    { label: 'Trade History', keywords: ['trade history', 'history', 'past trades', 'logs', 'records'], tab: 'trades', icon: '📋' },
    { label: 'Affirmations', keywords: ['affirmations', 'motivation', 'mindset', 'quotes', 'positive'], tab: 'affirmations', icon: '💪' },
    { label: 'Calendar', keywords: ['calendar', 'date', 'schedule', 'month', 'week', 'year'], tab: 'calendar', icon: '📅' },
    { label: 'Tools', keywords: ['tools', 'economic calendar', 'position size', 'calculator', 'forex'], tab: 'tools', icon: '🛠️' },
    { label: 'Account', keywords: ['account', 'profile', 'balance', 'currency', 'trading account'], tab: 'account', icon: '👤' },
    { label: 'Settings', keywords: ['settings', 'preferences', 'configuration', 'theme', 'setup'], tab: 'settings', icon: '⚙️' },
];

const searchableSettings = [
    { label: 'General Settings', keywords: ['general', 'theme', 'default account', 'risk per trade', 'leverage'], section: 'general', icon: '⚙️' },
    { label: 'Personal Info', keywords: ['personal info', 'name', 'display name', 'email', 'experience', 'profile'], section: 'personalInfo', icon: '👤' },
    { label: 'Preferences', keywords: ['preferences', 'compact view', 'confetti', 'auto-save', 'default trade view'], section: 'preferences', icon: '🎨' },
...
async function checkAndShowProfileModal() {
    try {
        if (!currentUser) return;
...
function fillPersonalInfoForm(profile) {
    ...
    if (experienceSelect) experienceSelect.value = profile.experienceLevel || 'intermediate';
...
function performSearch(query) {
    ...
    allTrades.forEach(trade => {
        const match =
            (trade.symbol?.toLowerCase().includes(lower)) ||
            (trade.notes?.toLowerCase().includes(lower)) ||
            (trade.mood?.toLowerCase().includes(lower)) ||
            (trade.type?.toLowerCase().includes(lower));
        if (match) {
            results.push({
                type: 'trade',
                label: `${trade.symbol} (${trade.type}) - ${formatCurrency(trade.profit)}`,
                data: trade,
                tab: 'trades',
                icon: '📈'
            });
    ...
    userAccounts.forEach(account => {
        if (account.name?.toLowerCase().includes(lower)) {
            results.push({
                type: 'account',
    ...
    // 3. Search Affirmations
    allAffirmations.forEach(aff => {
        if (aff.text?.toLowerCase().includes(lower)) {
            results.push({
                type: 'affirmation',
                label: `"${aff.text.substring(0, 45)}${aff.text.length > 45 ? '...' : ''}"`,
                data: aff,
                tab: 'affirmations',
                icon: '💬'
            });
    ...
    // 5. Search Static Settings
    searchableSettings.forEach(item => {
        const matchLabel = item.label.toLowerCase().includes(lower);
        const matchKeywords = item.keywords.some(k => k.toLowerCase().includes(lower));
        if (matchLabel || matchKeywords) {
            if (!results.some(r => r.type === 'setting' && r.label === item.label)) {
                results.push({
                    ...item,
                    type: 'setting',
                    label: item.label,
...
function renderSearchResults(results) {
    ...
    if (!results || results.length === 0) {
        container.innerHTML = `<div class="text-gray-500 text-center py-3 text-sm">No results found</div>`;
    ...
    typeOrder.forEach(type => {
        if (groups[type] && groups[type].length > 0) {
            html += `<div class="text-xs font-semibold text-gray-400 uppercase tracking-wider mt-3 first:mt-0 px-2">${typeLabels[type] || type}</div>`;
            groups[type].forEach(item => {
                html += `
                    <div class="search-result-item flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[rgba(201,168,76,0.08)] cursor-pointer transition-colors duration-150"
                         data-tab="${item.tab || ''}" 
                         data-section="${item.section || ''}" 
                         data-type="${item.type}" 
                         data-id="${item.data?.id || ''}">
                        <span class="text-base">${item.icon || '•'}</span>
                        <span class="text-gray-200 text-sm flex-1">${item.label}</span>
                        <span class="text-xs text-gray-500"><i class="fas fa-arrow-right"></i></span>
                    </div>
                `;
            });
        }
    ...
    container.querySelectorAll('.search-result-item').forEach(el => {
        el.addEventListener('click', function() {
            ...
            if (type === 'feature' || type === 'navigation') {
                ...
                document.getElementById('tradesTab')?.click();
            } else if (type === 'account') {
                document.getElementById('accountTab')?.click();
            } else if (type === 'affirmation') {
                document.getElementById('affirmationsTab')?.click();
            }
        });
    });
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', async (e) => {
            ...
            const experienceLevel = document.getElementById('profileExperience').value;
            if (!fullName) {
                alert('Full Name is required.');
                return;
            }
